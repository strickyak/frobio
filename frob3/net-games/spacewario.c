...
tx_ptr_t WizDataToSend(PARAM_SOCK_AND tx_ptr_t tx_ptr, const char* data,
                       size_t n);
tx_ptr_t WizBytesToSend(PARAM_SOCK_AND tx_ptr_t tx_ptr, const byte* data,
                        size_t n);
void WizFinalizeSend(PARAM_SOCK_AND const struct proto* proto, size_t n);
errnum WizSendChunk(PARAM_SOCK_AND const struct proto* proto, char* data,
                    size_t n);
...
void UdpDial(PARAM_SOCK_AND const struct proto* proto, const byte* dest_ip,
             word dest_port);
...
const struct sock Socks[2] = {
    {0x400, 0x4000, 0x6000}, {0x500, 0x4800, 0x6800},
...
void Enable1BitSound() {
  *(volatile byte*)0xFF23 &=
      ~0x04;  // Clear bit 2 to enable Data Direction access
...
void WizOpen(const struct sock* sockp, const struct proto* proto,
             word local_port) {
...
//////////////////////////////////////////////////////////////////
const char SixFFs[6] = {(char)0xFF, (char)0xFF, (char)0xFF,
...
tx_ptr_t WizBytesToSend(const struct sock* sockp, tx_ptr_t tx_ptr,
                        const byte* data, size_t n) {
  return WizDataToSend(SOCK_AND tx_ptr, (char*)data, n);
}
tx_ptr_t WizDataToSend(const struct sock* sockp, tx_ptr_t tx_ptr,
...
void WizFinalizeSend(const struct sock* sockp, const struct proto* proto,
                     size_t n) {
...
errnum WizSendChunk(const struct sock* sockp, const struct proto* proto,
                    char* data, size_t n) {
...
errnum WizRecvGetBytesWaiting(const struct sock* sockp,
                              word* bytes_waiting_out) {
  errnum e = WizCheck(JUST_SOCK);
  if (e) return e;
...
void ComputeGravity(word x, word y, int* gx_out, int* gy_out) {
  *gx_out = *gy_out = 0;
  ...
  if (tx >= W / 2 - W / 8 && ty >= H / 2 - H / 8) {
    ...
    word index = /* 2*(H/2)*(W/2) + */ (ix << 2) +
                 (iy << (2 + 3));  // four-byte records.
...
void CheckIncomingPackets() {
  ...
  if (e == OKAY) {
    ...
    if (h->udp_port == SPACE_WAR_PORT &&
        len == sizeof(struct broadcast_payload)) {
...
bool DetectHits(struct body* my_missile, byte my_num) {
  for (byte i = 0; i < NUM_SHIPS; i++) {
    ...
    word dx = (my_missile->x > p->x) ? (my_missile->x - p->x)
                                     : (p->x - my_missile->x);
