...
// Set the 6809 Direct Page register.
void SetDP(word dp_value) {
#ifdef __GNUC__
  asm volatile (
      "  ldd %0 \n"
      "  tfr b,dp"
      :                // No output
      : "m" (dp_value)  // Input a word.
      : "d"            // clobbers D
  );
#else
  asm {
...
errnum DoNetwork(byte a, byte b) {
  byte tail;
  if (!GetNum1Byte(&tail)) {
    // eschew mod // tail = 30 + (byte)(WizTicks() % 200u);  // random 30..229
    tail = 30 + Mod200(WizTicks());  // random 30..229
  }
  Vars->ip_addr[0] = a;
  Vars->ip_addr[1] = b;
  Vars->ip_addr[2] = 23;
