...
#define SOCK2_AND (Socks+2),
#define JUST_SOCK2 (Socks+2)
...
#define WAITER_DOMAIN_NAME "lemma.yak.net"

#define DNS_SERVER_PORT 53 /* named */
...
#define TYPE_A 1 // an ipV4 Host Address "A" record.
...
byte* encode_name(byte* buf, const char* s) {
    byte* sizep = buf;
    byte* p = (byte*) buf+1;
    while(true) {
        switch (*s) {
...
void SendDnsRequest(const struct sock* sockp, const char* query, word id) {
  // second is False for Discover, True for request.
  struct dns_header *h = (struct dns_header*)PACKET_BUF;
  memset((char*)PACKET_BUF, 0, sizeof *h);
  ...
  word packet_size = p - (byte*)PACKET_BUF;

  tx_ptr_t tx_ptr = WizReserveToSend(SOCK_AND packet_size);
  tx_ptr = WizBytesToSend(SOCK_AND tx_ptr, (byte*)PACKET_BUF, packet_size);
  WizFinalizeSend(SOCK_AND &UdpProto, packet_size);
}

byte* SkipName(byte* p) {
	while (*p) { // while len != 0
		p += 1 + *p; // skip len, and len bytes
	}
	return p+1; // position after len 0
}

errnum RecvDnsReply(const struct sock* sockp, word plen, word id, byte* addr_out) {
  errnum e = WizRecvChunk(SOCK_AND  (char*)PACKET_BUF, plen);
  if (e) return e;

  struct dns_header *h = (struct dns_header*)PACKET_BUF;
  if (h->id != id) return 91; // Wrong ID

  byte rcode = (byte)(h->qr_opcode & 0x000Fu);
  if (rcode) return 64 + rcode; // DNS Error

  if (h->an_count < 1) return 92; // No Answer

  byte* p = (byte*)(h+1);

  /* skip question */
  for (byte i = 0; i < (byte)h->qd_count; i++) {
  	p = SkipName(p);
  	p += 4; // Skip qtype & qclass of question
  }
  /* use the first answer that is an A record */
  for (byte i = 0; i < (byte)h->an_count; i++) {
  	p = SkipName(p);
  	if (((word*)p)[0] != TYPE_A) continue; // wrong type
  	if (((word*)p)[4] != 4) continue; // wrong RDLENGTH
	memcpy(addr_out, p+10, 4);
	return 0;
  }
  return 93; // No A record.
}

errnum ResolveDnsAddr(const char* query, byte* addr_out) {
  errnum e;
  struct UdpRecvHeader hdr;
  word id = (WizTicks() & 0x7FFEu) | 0x8000u;

  WizOpen(SOCK2_AND &UdpProto, id);
  UdpDial(SOCK2_AND  &UdpProto,
          /*dest_ip=*/ (const byte*)SixFFs, DNS_SERVER_PORT);

  for (byte i = 0; i < 10; i++) {
      PrintF(" *%d* ", i);

      SendDnsRequest(SOCK2_AND  query, id);
      Delay(10000);

      e = WizRecvChunkTry(SOCK2_AND  (char*)&hdr, sizeof hdr);
      if (e) continue;

      e = RecvDnsReply(SOCK2_AND  hdr.len, id, addr_out);
      if (!e) return 0;
  }
...
void OpenDiscoverySockets() {
  ...
  WizOpen(SOCK2_AND &BroadcastUdpProto, LAN_CLIENT_PORT);
  UdpDial(SOCK2_AND  &BroadcastUdpProto,
          /*dest_ip=*/ (const byte*)SixFFs, LAN_SERVER_PORT);
}
...
errnum DnsPhase() {
  ...
  for (byte i = 10; i < 20; i++) {
        PrintF(" *%d* ", i);
	e = ResolveDnsAddr(WAITER_DOMAIN_NAME, Vars->ip_waiter);
	PrintF(" =%d ", e);
	if (!e) return 0;
  }
  return e;
